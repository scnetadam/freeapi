/**
 * 龟钮印证 �?数据库连�?(PostgreSQL via CloudBase API)
...
function getCredentials() {
  ...
  // 优先从环境变量读�?  if (process.env.TCB_SECRET_ID && process.env.TCB_SECRET_KEY) {
...
async function query(sql, params = []) {
  ...
  // 需要先切换�?x402 数据�?  const response = await client.ExecutePGSql({
...
async function initSchema() {
  // 建表语句，列名匹�?dataStore.pg.js 中使用的 camelCase 带引号列�?  const schemaSql = [
